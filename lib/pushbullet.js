...
var dumpDeviceList = function (deviceList) {
    errors.logInfo("Available devices:");
    deviceList.forEach(function (device) {
       errors.logInfo(['  -', device.iden, '(', device.extras.manufacturer, device.extras.model, ')'].join(' ')); 
    });
};
...
var push = function(bullet) {
    ...
    getDevices().then(function(deviceList) {
        ...
        if (deviceList.length < 0) {
            errors.logWarn('No valid device identifier found!');
            return sent.reject();
        }
